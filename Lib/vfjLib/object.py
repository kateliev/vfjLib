                ...
                for key, value in obj.items():
                    if key == search:
                        ...
                        if isinstance(value, (dict, list)):
                            yield from extract_helper(value, search)

            elif isinstance(obj, list):
                for item in obj:
                    yield from extract_helper(item, search)
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if key == search and isisntance_plus(value, search_type):
                        ...
                        if isinstance(value, (dict, list)):
                            yield from where_helper(value, search)

            elif isinstance(obj, list):
                for item in obj:
                    yield from where_helper(item, search)
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if value == search_value and eq_plus(key == search_key, search_key):
                        ...
                        if isinstance(value, (dict, list)):
                            yield from where_helper(value, search_value)

            elif isinstance(obj, list):
                for item in obj:
                    yield from where_helper(item, search_value)
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if search in key and isisntance_plus(value, search_type):
                        ...
                        if isinstance(value, (dict, list)):
                            yield from contains_helper(value, search)

            elif isinstance(obj, list):
                for item in obj:
                    yield from contains_helper(item, search)
...
class attribdict(defaultdict):
    ...
    def __init__(self, *args, **kwdargs):
        super().__init__(attribdict, *args, **kwdargs)
    ...
    def dir(self):
        tree_map = [f"   .{key}\t{type(value)}" for key, value in self.items()]
